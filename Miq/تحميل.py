...
async def song(client, message: Message):
    ...
    if not urlissed:
        ...
        return
    pablo = await client.send_message(message.chat.id, f"**🔎 جاري تحميل : @algafoyr ** `{urlissed}`")
...
async def vsong(client, message: Message):
    ...
    pablo = await client.send_message(message.chat.id, f"**🔎 يبحث عن : @algafoyr** `{urlissed}`")
    if not urlissed:
    ...
    try:
        ...
        await event.edit(event, f"**التحميل فشل : @algafoyr** \n `{str(e)}`")
        return
    c_time = time.time()
